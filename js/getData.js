...
function SaveS() {
    localStorage.setItem('sideDisp', JSON.stringify({
        disp: sidebar.style.display
    }));
  }
  
  function restoreside() {
    let side = JSON.parse(localStorage.getItem('sideDisp'));
    if (side) {
...
let priceV=JSON.parse(localStorage.getItem("rangeR"))
function getData(data) {
  box.innerHTML = "";
  data.forEach((el) => {
    if ((shop_f == el.company || shop_f == "all")) {
      if (el.price<=priceV.home) {
        {
          let card = document.createElement("div");
          card.classList.add("card");
          let image = document.createElement("img");
